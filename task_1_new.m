...
img_path = 'imgs\7.jpg';
img = imread(img_path);
frac_dim_level_min = 2.45;
frac_dim_level_max = 3.1;
eig_cor_matrix_level = 7700;
level = 16;
...
if (answ == 1)
    if (exist('True_DO_coefs.csv', 'file') == 2)&&(exist('All_DO_coefs.csv', 'file') == 2)
        ...
        Answer = [];
        True_DO_coefs = table(Number_on_img,Img_name,X,Y,Width, High, Mean_brightness, Img_std, Wc_coef_level,Fractal_dim_level_min,Fractal_dim_level_max,Eigenvalue_of_matrix_level,Fractal_dim,Eigenvalue_of_matrix,Answer);              
        All_DO_coefs = True_DO_coefs;                        
    end
    for i = 2:A 
        cell_DO = {i-1, img_name, v(i,3), v(i,1), v(i,4)-v(i,3), v(i,2)-v(i,1),mean_bright, img_std, level, frac_dim_level_min,frac_dim_level_max, eig_cor_matrix_level, v(i,6),v(i,5), e(i, 2)};
